...
const dashboard = () => {
  ...
  const[showUploadResume, setShowUploadeResume] = useState(false)
  ...
  const deleteResume = async (resumeid) => {
     const confirm = window.confirm('Are you you want to delete this resume?')
     if(confirm){
              ...
              '>
                <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                  ...
                  <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required
                   /> 
                    ...
                    <div>
                      <label  htmlFor="resume-input"  className='block text-sm text-slate-700'> select resume file
                        <div className=' flex flex-col items-center justify-center gap-2 border group text-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer treansition-colors
                        '>
                          {resume ?(
                   ...
                   <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors
                   ' onClick={()=>
                   {setShowUploadeResume(false);setTitle('')}
                   }/>
                </div>
              </form>
